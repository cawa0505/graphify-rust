...
^- [x] 1.1 `QdrantConfig` 新增 `local_fallback_enabled`、`local_bin_dir`、`local_storage_dir`、`local_version`、`local_http_port`、`local_grpc_port`（全數 `#[serde(default = ...)]`，既有 TOML 零遷移）
^- [x] 1.2 為新欄位補 serde 解析測試（default 值 + 顯式值 + 相容舊設定檔無欄位）

## 2. QdrantLocalProcess 模組（graphify-memory/src/local_process.rs）

^- [x] 2.1 新增 deps：`flate2`、`tar`（.tar.gz 解壓）
^- [x] 2.2 實作 binary 下載：GitHub Releases asset URL 組裝（平台 target 選擇，x86_64-linux-gnu 預設）+ `.tar.gz` 解壓至 `local_bin_dir`
^- [x] 2.3 實作 SHA-256 驗證：GitHub Releases API `digest` 欄位比對，不符回 Err 且不執行未驗證 binary
^- [x] 2.4 實作 spawn：`std::process::Command` + `QDRANT__` env 覆寫（HTTP/GRPC port、STORAGE_PATH、TELEMETRY_DISABLED、LOG_LEVEL）
^- [x] 2.5 實作 readiness：`/healthz` 輪詢（bounded timeout ~30s / 250ms 間隔）
^- [x] 2.6 實作 shutdown：`Drop` SIGTERM + `wait()`，無 orphan
^- [x] 2.7 單元測試：下載+驗證（digest 不符拒絕執行）、spawn+healthz、Drop 終止（可注入 fake binary / 非網路測試路徑）

## 3. 雙軌 StorageMode + init_with_fallback（graphify-memory/memory.rs）

^- [x] 3.1 新增 `StorageMode::{ServerUrl(String), LocalProcess}` 並掛到 store 狀態
^- [x] 3.2 實作 `init_with_fallback(lt_config, concurrency)`：Server `/healthz` 探測（10ms bounded）→ healthy 走 `ServerUrl`、否則 spawn Local → `LocalProcess`
^- [x] 3.3 `local_fallback_enabled=false` 時完全走現行行為（`new()` 向後相容，等價語意）
^- [x] 3.4 Local spawn 失敗 → `MemoryStatus::Unavailable` 語意，不硬崩潰
^- [x] 3.5 保留 `new()` 同步建構（內建 block_on 橋接或標註 deprecated 指引），不破壞 graphify-mcp/graphify-cli 既有呼叫
^- [x] 3.6 測試：Server healthy 選 ServerUrl / Server 不可用降級 Local / fallback disabled 維持 unavailable（fake health 注入）
