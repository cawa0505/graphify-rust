...
- **GIVEN** 使用者明確在非 git 目錄的 `$HOME` 本身執行 `relay_init`
- **WHEN** init 成功建立 relay root（workspace root = cwd = `$HOME`）
- **THEN** 輸出附加警告：該狀態檔將被所有於 `$HOME` 啟動的 session 使用
