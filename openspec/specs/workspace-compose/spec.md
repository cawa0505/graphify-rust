# workspace-compose Specification

## Purpose

跨 workspace 組裝（Assembly）：YAML Assembly Manifest 描述多個 workspace 的語意關聯，合併為 unified graph 並投影為 box-of-rain ASCII/SVG 圖。Manifest 是「組裝說明書」而非系統唯一真相；同一批 workspace 可存在多份 manifest 代表不同視角。
...
- 任務 1.1/1.2 feasibility spike 記錄於 `openspec/changes/workspace-compose`（歸檔前）design.md D4/D6。
- 6.1 e2e narrative 與 6.2 驗證數據（180 tests、clippy 0 warnings、fmt 乾淨）保留於 `openspec/changes/workspace-compose/verification.md`（歸檔前）。
- spec 綁定測試：compose_manifest 15、compose_merge 17、mcp compose 4（三個 validate scenario + 原子 write 不可變性 + 前綴去重 + 投影形狀剛性存在）。
