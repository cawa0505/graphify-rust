# handoff-doctor — Relay 狀態檔健康檢查（doctor）
...
- 2026-09-24 實作完成（change `handoff-doctor`，實作於 GraphifyPlugins/graphify-plugin-handoff `src/doctor/` + GraphifyRust `graphify-cli`）：
  - `cargo test -p graphify-plugin-handoff`：84 passed / 0 failed（doctor 情境 15 測試：foreign bare-name、monorepo 不誤殺、legacy 自身條目不誤殺、絕對路徑 root 外、unparseable、$HOME stray、scan 孤兒、legacy INFO、多髒全列、fix 只刪 dirty、fix 前列清單、scan 多 repo、no-state、fix 後退出碼、掃描根不存在）
  - `cargo test -p graphify-cli`：24 passed / 0 failed；兩 workspace clippy 0 警告、fmt 通過
  - e2e 實跑：退出碼契約（no-state→0、dirty --scan→1、fix 成功→0、掃描根不存在→2）；實際 repos 環境 `--scan`：9 檔全 INFO、0 DIRTY、EXIT=0，與人工清理結論一致
  - e2e 抓到並修正誤殺：legacy 自身條目（key == root basename、path 同名裸名）首版誤判 foreign DIRTY；spec 檢查項 2(c) 增補自身條目豁免 + 剛性測試後重掃零誤殺
  - `openspec validate handoff-doctor --strict`：valid
