# rust-tuikit Specification

## Purpose

通用 Rust TUI 範本層（獨立 repo `../RustTuiKit`）：提供 theme/layout/log/modal/flash/component 六模組，供 GraphifyRust 與其他 Rust TUI 專案重用，零 graphify 依賴。
...
- `cargo test --workspace`：167 通過（kit 11 + cli 26，含 TestBackend 位元級渲染一致性斷言）
- 隱私審計（規則 #2448）：無 IP/hostname/個人路徑/key
- TUI live 目檢：使用者初驗通過（2026-09-21）
- RustTuiKit repo：gitlab.com:saaslab/rust-tuikit main 已推送
