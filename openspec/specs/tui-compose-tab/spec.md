...
- `cargo test --workspace` 全綠（graphify-tui 單元測試 `test_split_ref_plain_and_qualified`、`test_render_compose_ascii_boxes_and_edges` 通過；全 workspace 0 failed）
- 端到端煙霧測試：臨時 manifest + 兩個 workspace graph → `compose validate/graph/render` 全通
- `clippy` 全 workspace 乾淨、`cargo fmt` 已套用
- TUI `y` 鍵 Compose panel 實測：menu → diagram → Esc 逐層返回正常
