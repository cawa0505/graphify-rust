...
- **Domain memory envelope**: plugin records use `graphify_core::plugin_memory::PluginMemoryEnvelope<T>` (`format_version` / `workspace_key` / `plugin_id` / `record_id` / `record_kind` / `created_at` / `source_refs` / `payload`) with plugin-specific payloads (`OpenDocPayload`, `ReviewPayload`, `HandoffPayload`), each independently versioned via `schema_version`. Storage isolation and name derivation live in `graphify-memory::plugin_memory`; the core defines only the data contract. `HandoffPayload.reconstructable_query_metadata` is a strongly-typed `MemoryQueryCriteria` (target symbols / domain categories / search terms), and session tracking wraps it in `HandoffSnapshot` (`snapshot_id` / `session_id` / `workspace_key` / `created_at` / `expires_at` / `payload`) — no Qdrant point IDs anywhere.
